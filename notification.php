...
if(isset($_GET['id'])){
  $main = $_GET['id'];
  $sql_update = mysqli_query($con, "UPDATE location_admin SET status = 1 WHERE id = '$main'");
}
?>
<!DOCTYPE html>
<html lang="en">
...
$sql = "SELECT *, sub_admin.id AS sid, location_admin.id AS lid FROM location_admin 
LEFT JOIN sub_admin ON sub_admin.id=location_admin.sub_admin_id Where user_id = '$id' AND status ='0'";
...
while($row = $query->fetch_assoc()){ ?>
	  <ul class="list-group list-group-flush">
		...
		<button><a href="admin_location.php?ID=<?php echo $row['lid'];?>">Locate</i></a></button><br>
		<?php echo $row['date'];?><br>
		The rescuer is at <?php echo $row['address'];?><br>
		</li>
	  </ul>
	  <?php
				}
