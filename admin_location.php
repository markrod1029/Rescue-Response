...
<head>
	...
	<title>Notification-Locate Admin</title>
    <script async src="https://www.google-analytics.com/analytics.js"></script>
...
<a href="notification.php" class="notification">
	...
	<span class="badge bg-danger" id="counts" style="color:white;  border-radius: 60%;position: relative;top: -10px;left: -7px;font-size: 8px;"><?php echo $count['total']; ?></span>
		...
		<main>
            ...
            <?php
                $id = $user['id'];
                $view = "SELECT *, sub_admin.id AS sid, location_admin.id AS lid FROM location_admin 
                LEFT JOIN sub_admin ON sub_admin.id=location_admin.sub_admin_id where user_id ='$id'";
                $result = $conn->query($view);
                $row = $result->fetch_assoc();
                ?>  
                <h3 class="section-head">Rescuer's Current Location<span class="las la-map-marked-alt"></span></h3> 
...
.announce h3 button:first-child{
    background-color: rgb(4, 134, 67);
}
.announce h3 button:nth-child(2){
